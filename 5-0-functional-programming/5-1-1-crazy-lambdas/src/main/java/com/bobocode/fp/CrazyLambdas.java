...
public class CrazyLambdas {
    ...
    /**
     * Returns a {@link UnaryOperator} that accepts str to str function and returns the same function composed with a
     * modification that removes all spaces, including middle spaces, and trims the result.
     *
     * @return function that composes functions with trimming and removing all spaces
     */
    public static UnaryOperator<Function<String, String>> composeWithTrimFunction() {
        return func -> func.andThen(str -> str.replaceAll("\\s+", "").trim());
    }
    ...
    public static <T, U extends Comparable<? super U>> Comparator<T> comparing(Function<? super T, ? extends U> mapper) {
        return (t1, t2) -> mapper.apply(t1).compareTo(mapper.apply(t2));
    }
    ...
    public static <T, U extends Comparable<? super U>> Comparator<T> thenComparing(
        ...
        return (t1, t2) -> {
            int comparison = comparator.compare(t1, t2);
            if (comparison == 0) {
