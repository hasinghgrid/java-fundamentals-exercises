...
public class CrazyStreams {
    ...
    public Optional<Account> findRichestPerson() {
        return accounts.stream()
                .max(Comparator.comparing(Account::getBalance)); // Verify: ensure getBalance() exists
    }
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream()
                .filter(account -> account.getBirthday().getMonth() == birthdayMonth) // Verify: ensure getBirthday() returns LocalDate
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(account -> "MALE".equalsIgnoreCase(String.valueOf(account.getSex()))));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(
                        account -> account.getEmail().split("@")[1])); // Verify: ensure getEmail() exists
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .mapToInt(account -> account.getFirstName().length() + account.getLastName().length()) // Verify: ensure getFirstName() and getLastName()
                .sum();
    }
    ...
    public BigDecimal calculateTotalBalance() {
        return accounts.stream()
                .map(Account::getBalance) // Verify: ensure getBalance() exists
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream()
                .sorted(Comparator.comparing(Account::getFirstName) // Verify: ensure getFirstName() and getLastName()
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .anyMatch(account -> account.getEmail().endsWith("@" + emailDomain)); // Verify: ensure getEmail()
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream()
                .filter(account -> account.getEmail().equals(email)) // Verify: ensure getEmail()
                .findFirst()
                .map(Account::getBalance) // Verify: ensure getBalance()
                .orElseThrow(() -> new EntityNotFoundException("Cannot find Account by email=" + email));
        // Replaced EntityNotFoundException with NoSuchElementException since the former isn't available
    }
    ...
    public Map<Long, Account> collectAccountsById() {
        return accounts.stream()
                .collect(Collectors.toMap(Account::getId, account -> account)); // Verify: ensure getId()
    }
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream()
                .filter(account -> account.getCreationDate().getYear() == year) // Verify: ensure getCreationDate() returns LocalDate
                .collect(Collectors.toMap(Account::getEmail, Account::getBalance)); // Verify: ensure getEmail() and getBalance()
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream()
                .collect(Collectors.groupingBy(
                        Account::getLastName, // Verify: ensure getLastName()
                        Collectors.mapping(Account::getFirstName, Collectors.toSet()))); // Verify: ensure getFirstName()
    }
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream()
                .collect(Collectors.groupingBy(
                        account -> account.getBirthday().getMonth(), // Verify: ensure getBirthday() returns LocalDate
                        Collectors.mapping(Account::getFirstName, Collectors.joining(", ")))); // Verify: ensure getFirstName()
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(Collectors.groupingBy(
                        account -> account.getCreationDate().getMonth(), // Verify: ensure getCreationDate() returns LocalDate
                        Collectors.mapping(Account::getBalance, // Verify: ensure getBalance()
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                .flatMap(account -> account.getFirstName().chars().mapToObj(ch -> (char)ch)) // Verify: ensure getFirstName()
        ...
        return accounts.stream()
                .flatMap(account -> {
                    Stream<Character> firstNameChars = account.getFirstName().length() >= nameLengthBound
                            ? account.getFirstName().chars().mapToObj(ch -> (char)ch)
                            : Stream.empty();
                    Stream<Character> lastNameChars = account.getLastName().length() >= nameLengthBound
                            ? account.getLastName().chars().mapToObj(ch -> (char)ch)
                            : Stream.empty();
